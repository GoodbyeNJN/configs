...
const isEmpty = (data: object | string | undefined): boolean => {
    if (data === undefined) {
        return true;
    }
...
const isNumber = (data: unknown): data is number => typeof data === "number" && !Number.isNaN(data);
...
const isString = <T>(data: T | string): data is NarrowedTo<T, string> => typeof data === "string";
